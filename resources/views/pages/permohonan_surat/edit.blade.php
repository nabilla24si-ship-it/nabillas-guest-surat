...
@section('title', 'Edit Permohonan Surat - Layanan Mandiri Desa')
    ...
    <div class="container py-5">
        <div class="row justify-content-center py-5">
            <div class="col-lg-10 pt-lg-5 mt-lg-5 text-center">
                <h1 class="display-4 text-white animated slideInDown">Edit Permohonan Surat</h1>
    ...
    <div class="container">
        <div class="card shadow border-0">
            <div class="card-header bg-primary text-white py-3">
                <h5 class="mb-0">
                    <i class="fas fa-edit me-2"></i>Form Edit Permohonan Surat
                </h5>
            </div>
            <div class="card-body">
                @if ($errors->any())
                ...
                <form action="{{ route('permohonan-surat.update', $permohonan->permohonan_id) }}" method="POST">
                    ...
                    <div class="row mb-3">
                        <div class="col-md-6">
                            ...
                            <select name="pemohon_warga_id" class="form-select @error('pemohon_warga_id') is-invalid @enderror" required>
                                ...
                                @foreach($warga as $w)
                                    <option value="{{ $w->warga_id }}" {{ old('pemohon_warga_id', $permohonan->pemohon_warga_id) == $w->warga_id ? 'selected' : '' }}>
                                        {{ $w->nama }} - {{ $w->nik }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="jenis_id" class="form-select @error('jenis_id') is-invalid @enderror" required>
                                ...
                                @foreach($jenisSurat as $jenis)
                                    <option value="{{ $jenis->jenis_id }}" {{ old('jenis_id', $permohonan->jenis_id) == $jenis->jenis_id ? 'selected' : '' }}>
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="text" class="form-control" value="{{ $permohonan->nomor_pemohonan }}" readonly>
                            <small class="form-text text-muted">Nomor permohonan tidak dapat diubah</small>
                        </div>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea name="catatan" class="form-control @error('catatan') is-invalid @enderror" rows="3"
                                  placeholder="Masukkan catatan tambahan (opsional)">{{ old('catatan', $permohonan->catatan) }}</textarea>
                        @error('catatan')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
